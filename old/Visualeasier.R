                ...
                navbarPage("Visualeasier", # Project Title
                           ...
                           tabPanel("Cluster Map", # Initialization tab, responsible for Cluster Map.
                                    sidebarPanel( h2("Input data", align = "center"),
                                                  numericInput("ncluster", "Insert number of clusters: ", 0, min = 0, max = NA, step = 1), # Input for number of clusters
                                                  ...
                                                  hr(),
                                                  downloadButton("download_cluster_table", "Download Cluster Table Data"), # Download outputs
                                    ...
                                    mainPanel(h3(HTML("<b>Preview </b>"), align = "center"), # Main panel, where plots will be displayed
                                              fluidRow(
                                                column(12,
                                                       uiOutput("graph_cluster") # Cluster Map plot
                                                )
                                              ),
                                              br(),
                                              fluidRow(
                                              ...
                                              fluidRow(
                                                column(12,
                                                       tableOutput("table_cluster") # Cluster table
                                                       )
                                              ),
                                              fluidRow(
                                                column(12,
                           ...
                           tabPanel("Help", "This application is designed to help users visualize Geochemical analysis results.",
                                    hr(),
                                    "Visualeasier is divided in two main tabs, the Elemental Map tab is responsible for single-element plot visualisation and combined plot download. The Filtered Map tab is designed for raster analysys, map a single element or generate a ternary map and apply the desired filter",
                                    hr(),
...
server <- function(input, output, session) {
    ...
    graph_list <- lapply(1:num, function(i) { # First input transformation to raster data
      ...
      ggplot(df, aes(x,y, fill = norm)) + # Plotting
        ...
        ggplot(df, aes(x,y, fill = norm)) + # Plotting
          ...
          geom_segment(aes(x = x_start, y = y_pos, xend = x_end, yend = y_pos), color = "black", size = 1)+
          annotate("text", x = text_x, y = text_y, label = paste0("5 mm"), size = 3)
        }
    })
    ...
    if (input$type == 1) {
      ...
      if (filter == "None") {
        ...
        p <- ggplot(df) +
          ...
          annotate(geom = "label", x=((max(df$x))/2)-50, y= max(df$y) + 20, label = elements_selectedR()[1], fill = "red", fontface = "bold", color = "white")+
          annotate(geom = "label", x=((max(df$x))/2), y= max(df$y) + 20, label = elements_selectedR()[2], fill = "green", fontface = "bold", color = "white")+
          annotate(geom = "label", x=((max(df$x))/2)+50, y= max(df$y) + 20, label = elements_selectedR()[3], fill = "blue", fontface = "bold", color = "white")
  ...
  table_cluster_reactive <- reactive({ # Cluster table
    ...
    mean_cols <- paste0(start_col, "_mean")
    sd_cols <- paste0(start_col, "_sd")
    intercalated_cols <- as.vector(rbind(mean_cols, sd_cols)) 
    ...
    cluster_norm %>%
      ...
      dplyr::select(Cluster, Percentage, dplyr::all_of(intercalated_cols))
